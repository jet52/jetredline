...
@pytest.mark.parametrize("model_id,expected", [
    ...
    # The raw id keeps its context-window suffix; only the friendly half drops it.
    ("claude-opus-5[1m]", "Claude Opus 5 (claude-opus-5[1m])"),
    ("us.anthropic.claude-opus-4-5-v1:0",
     "Claude Opus 4.5 (us.anthropic.claude-opus-4-5-v1:0)"),
])
def test_describe_model_derives_the_friendly_half(model_id, expected):
    assert provenance.describe_model(model_id) == expected


@pytest.mark.parametrize("model_id", [
    "claude-3-5-haiku-latest",  # family token not where the pattern expects it
    "some-internal-build",
    "",
])
def test_describe_model_falls_back_to_the_bare_id(model_id):
    """Never invent a marketing name for a model shape it doesn't recognize."""
    assert provenance.describe_model(model_id) == model_id


def test_detect_model_reads_the_transcript(tmp_path):
    path = tmp_path / "t.jsonl"
    path.write_text(json.dumps({
        "type": "assistant",
        "message": {"model": "claude-opus-5", "content": []},
    }) + "\n", encoding="utf-8")
    assert provenance.detect_model(transcript=path) == "claude-opus-5"


def test_detect_model_returns_none_without_a_transcript(tmp_path):
    assert provenance.detect_model(transcript=tmp_path / "missing.jsonl") is None
    assert provenance.detect_model(session_id="") is None


def test_stamp_uses_the_detected_model(tmp_path, monkeypatch):
    """--model is optional: the footer's headline model comes from the same
    transcript the delegated-pass rows do, not from a self-report."""
    ...
    monkeypatch.setattr(sys, "argv", [
        "provenance.py", "--file", str(report), "--transcript", str(transcript),
    ])
    assert provenance.main() == 0
    assert "Claude Opus 4.8 (claude-opus-4-8)" in report.read_text()


def test_undetectable_model_is_an_error_not_a_blank_footer(tmp_path, monkeypatch):
    report = tmp_path / "R.md"
    report.write_text("# Report\n\nBody.\n", encoding="utf-8")
    monkeypatch.setattr(sys, "argv", [
        "provenance.py", "--file", str(report),
        "--transcript", str(tmp_path / "missing.jsonl"),
    ])
    assert provenance.main() == 2
    assert "generated by" not in report.read_text()


def test_explicit_model_overrides_detection(tmp_path, monkeypatch):
    report = tmp_path / "R.md"
    report.write_text("# Report\n\nBody.\n", encoding="utf-8")
    transcript = tmp_path / "t.jsonl"
    transcript.write_text(json.dumps({
    ...
    assert provenance.main() == 0
    assert "Claude Sonnet 5 (claude-sonnet-5)" in report.read_text()
