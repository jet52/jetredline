...
Design: every part of the footer is sourced deterministically — the skill
version (from SKILL.md frontmatter `version:`, then a VERSION file), the date
(the system clock), and the model (this session's transcript, read through
check_model.py — the same source the delegated-pass rows below the sentence
already use). The script never asks the model to name its own release, because
that self-report is exactly the channel a validation record must not depend on:
until it read the transcript, the sentence's headline model was a self-report
while the table under it was not. --model overrides, for a run with no
transcript on disk.
...
Usage:
    python3 provenance.py --file REPORT.md --subagents
...
normally needs to pass only --file.
"""
...
_MODEL_SUFFIX_RE = re.compile(r"\[[^\]]*\]\s*$")


def describe_model(model_id: str) -> str:
    """`claude-opus-4-8` -> `Claude Opus 4.8 (claude-opus-4-8)`.

    The exact id is the half a validation record needs; the friendly half is
    cosmetic, so it is derived mechanically from the id and falls back to the
    bare id for any shape this does not recognize, rather than inventing a
    marketing name for a model it has never heard of.
    """
    raw = (model_id or "").strip()
    ident = _MODEL_SUFFIX_RE.sub("", raw)
    lowered = ident.lower()
    if "claude" in lowered:
        ident = ident[lowered.index("claude"):]
    ...
    if not version:
        return raw
    return f"Claude {parts[1].title()} {version} ({raw})"


def detect_model(session_id=None, transcript=None):
    """Runtime model id from the session transcript, or None.

    check_model.py is an optional dependency: it ships beside this script in
    jetredline and jetmemo but not in every skill that vendors provenance, so
    an absent module degrades to "pass --model", never a crash.
    """
    try:
        import check_model
    except ImportError:
        return None
    path = Path(transcript) if transcript else check_model.find_transcript(session_id)
    if path is None:
        return None
    return check_model.model_from_transcript(path)
...
def main() -> int:
    ...
    ap.add_argument("--model", default=None,
                    help='Runtime model identifier, e.g. "Claude Opus 4.8 '
                         '(claude-opus-4-8)". Omit to read it from this '
    ...
    model = args.model
    if not model:
        detected = detect_model(args.session_id, args.transcript)
        if not detected:
            print("provenance: could not read the runtime model from the "
                  'transcript; re-run with --model "<name> (<id>)".',
                  file=sys.stderr)
            return 2
        model = describe_model(detected)

    version = read_version(Path(args.skill_dir))
    stamp(path, model, args.skill, version, args.date, rows)
    detail = f" | {len(rows)} delegated pass(es)" if rows else ""
    print(f"Stamped provenance: {args.skill} v{version} | {model} | {args.date}{detail}")
    return 0
