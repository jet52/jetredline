...
|---|---|---|---|---|---|---|
| 1 — Jurisdiction | claude-opus-4-8[1m] | 2 | 3.3k | 44.5k | 6 | 1m 04s |
| 4 — Fact check | claude-opus-4-8[1m] | 2 | 6.6k | 91.2k | 36 | 15m 11s |

The script reads those figures from this session's transcript (located via `$CLAUDE_CODE_SESSION_ID`), identifying each pass by the `references/pass-instructions/passN-*.md` path in its Task prompt — **so keep that path in the delegated prompts verbatim.** Only numbers and the model ID cross into the report; no prompt or result text does. The table is omitted whenever nothing was delegated (Web mode, inline mode, short opinions run in main context) and whenever the transcript can't be read — the transcript is an undocumented internal format, so a missing table is expected behavior, never an error to chase. Do **not** hand-write this table from your own recollection of the passes; if the script omits it, it stays omitted.
